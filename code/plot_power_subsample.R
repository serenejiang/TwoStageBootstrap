...
plot.power.subsample = function(N.x, N.y, n.x, n.y, B, m, alpha.qs, mean.x, mean.y, sd.x, sd.y, dist.x, dist.y, stat.test, repeats){
	...
	for (i in 1:m){
		...
		for (i in n.x){
			# results.H0 = stat.H0(group.x = dat$X, group.y = dat$Y, subsample.x = i, subsample.y = i, numboot = B, alpha = alpha.qs)
			# T.H0 = results.H0$T.stat.H0
			# cutoff.high = results.H0$cutoff.high
			# cutoff.low = results.H0$cutoff.low

			# T.H1 = stat.H1(group.x = dat$X, group.y = dat$Y, subsample.x = i, subsample.y = i, numboot = B)
